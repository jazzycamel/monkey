...
std::shared_ptr<ExpressionStatement> Parser::_parseExpressionStatement() {
    ...
    if (_peekTokenIs(SEMICOLON)) _nextToken();
...
std::shared_ptr<Expression> Parser::_parseExpression(int precedence) {
    ...
    while (!_peekTokenIs(SEMICOLON) && precedence < _peekPrecedence()) {
        ...
        if (infix == nullptr) return leftExpression;
        _nextToken();
...
std::shared_ptr<Expression> Parser::_parseIntegerLiteralExpression() {
    ...
    try {
        ...
        auto msg = string_format("Could not parse %s as integer", _currentToken.literal.c_str());
...
std::shared_ptr<Expression> Parser::_parseInfixExpression(std::shared_ptr<Expression> left) {
...
void Parser::_noPrefixParseFnError(const TokenType &t) {
    auto msg = string_format("No prefix parse function for '%s' found", t.c_str());
    _errors.push_back(msg);
}
...
void Parser::_peekError(const TokenType &t) {
    auto msg = string_format(
            ...
            t.c_str(),
            _peekToken.type.c_str()
    );
    _errors.push_back(msg);
}

int Parser::_peekPrecedence() const {
    try { return precedences.at(_peekToken.type); }
    catch (const std::out_of_range &oor) { return LOWEST; }
}

int Parser::_currentPrecedence() const {
    try { return precedences.at(_currentToken.type); }
    catch (const std::out_of_range &oor) { return LOWEST; }
}