...
void Lexer::_readChar() {
    if (_readPosition >= _input.length()) _ch = 0;
    else _ch = _input[_readPosition];
    _position = _readPosition;
    _readPosition++;
}

char Lexer::_peekChar() {
    if (_readPosition >= _input.length()) return 0;
    return _input[_readPosition];
}

std::string Lexer::_readNumber() {
    auto position = _position;
    while (_isDigit(_ch)) _readChar();
    return _input.substr(position, _position - position);
}

std::string Lexer::_readIdentifier() {
    auto position = _position;
    while (_isLetter(_ch)) _readChar();
    return _input.substr(position, _position - position);
}

bool Lexer::_isDigit(char ch) {
    return '0' <= ch && ch <= '9';
}
